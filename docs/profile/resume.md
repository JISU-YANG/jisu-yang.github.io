...
저는 겉으로 드러난 현상만 보지 않고, 그 이면에 무엇이 더 있는지를 끊임없이 관찰하고 분석하며 깊이 상상합니다.  단순히 문제를 해결하는 것이 아니라, 왜 그런 문제가 생겼고, 다시 발생하지 않으려면 무엇을 바꿔야 하는지를 고민합니다.

그런 점에서 개발은 제게 단순한 직무가 아니라, 일상 속 불편함을 해결하고, 아이디어를 현실로 구현하는 가장 현실적인 방법이라고 느꼈습니다. 관찰과 사색이 아이디어로 이어지고, 그것이 실체화되는 순간만큼 가슴 설레는 일은 없는 것 같습니다.
...
유년기부터 컴퓨터와 가까이 지내며 자랐고, 디지털 변화의 한가운데에서 성장해왔습니다. 중학교 시절, 스티브 잡스가 아이폰을 처음 발표하던 프레젠테이션을 보고 강한 충격을 받았습니다. 상상도 못 한 무언가로 사람들의 삶을 바꾸는 모습을 보며, 언젠가는 나도 ‘그런 무언가를 만드는 사람’이 되고 싶다는 꿈을 품게 되었습니다.

처음에는 디자인으로 시작해 프리랜서로 활동하며 사용자 경험을 고민했고, 팀 프로젝트를 기획하면서 자연스럽게 커뮤니티와 웹 서비스를 만들고 싶다는 바람이 생겼습니다. 그 과정에서 팀의 개발자가 이탈하면서 직접 PHP와 JavaScript를 수정하게 됐고, 한 줄을 고치기 위해 10시간 넘게 밤을 새우기도 했지만, 결국 해결했을 때의 성취감은 아직도 잊히지 않습니다. 그 경험을 계기로 ‘만들고 싶은 걸 직접 구현할 수 있다면 더 많은 것을 발명할 수 있겠다’는 생각이 들었고, 결국 본격적으로 개발자의 길을 걷기로 마음먹게 되었습니다.

이후 Java와 백엔드 기술을 중심으로 공부를 시작했고, 실제로 개발을 진지하게 배우면서 철학적이고 논리적인 구조와 이론에 매료되기도 했습니다. 취업에서의 시행착오와 학력에 대한 편견을 마주하면서도 자발적으로 팀을 이끌고, 기록하고, 함께 성장해나갔습니다. 그래서 지금은 개발을 단순한 기술로 보지 않고, 문제를 이해하고 구조를 설계하며, 협업과 커뮤니케이션까지 포함해 하나의 ‘과정’으로 받아들이고 있습니다.

저는 여전히 “사람들에게 의미 있는 무언가를 만들고 싶다”는 그 마음을 가지고 있고, 합리적이고 논리적인 것을 좋아하는 제 성향이 개발이라는 일과도 잘 맞닿아 있다고 느낍니다. 그래서 저는 개발자가 꼭 되어야 한다고, 그리고 잘할 수 있다고 확신합니다.
...
최근 맡았던 알림 도메인은 처음엔 단순히 Slack DM을 전송하는 수준이었지만, 서비스가 확장되며 다양한 구조적 문제가 드러나기 시작했습니다.

유사 API의 중복, 하드코딩된 메시지, 과도한 서비스 책임 분산 등은 유지보수성과 신뢰성을 떨어뜨리고 있었습니다.

저는 중복된 API를 단일 통합 API로 추상화하고, 메시지 템플릿은 DB 기반으로 분리하여 관리 효율을 높였습니다.

또한 전처리, 템플릿 바인딩, 히스토리 저장, 메시지 전송을 각기 독립된 서비스로 분리하고, Kafka 기반 이벤트 처리 구조를 도입해 흐름을 재구성했습니다.

중요한 알림 누락에 대비해 Fallback 이메일 발송 기능을 추가하고, 설정 서버를 통해 가용성과 유연성도 개선했습니다.

이 경험은 단순 기능 구현을 넘어, 시스템 전반을 개선하며 복잡한 문제의 본질을 파악하고 해결하는 역량을 키운 의미 있는 프로젝트였습니다.
...
같은 프로젝트에서 저는 Notion을 활용해 업무 분배와 문서 정리를 주도했습니다. 기획이나 설계 내용을 함께 논의한 후, 명세서·가이드·구조도 형태로 다듬어 GitHub Wiki에 올려 공식 기준처럼 활용했습니다. 

기능 우선순위나 담당자, 데일리 스크럼 기록 같은 내용을 문서로 남긴 덕분에, 누구나 항상 현재 상황을 파악할 수 있었고, 작업 리마인드나 협업 진행이 훨씬 수월했습니다.

특히 Git 브랜치 전략, 커밋 컨벤션, 네이밍 룰, 테스트 환경 설정 같은 반복적으로 혼란을 주는 요소들은 문서로 정리해두고, 코드 리뷰나 QA에서도 쉽게 참조할 수 있도록 했습니다.

복잡한 설정이나 기술 흐름은 담당자가 직접 작성하고 공유하도록 했고, 각자 경험한 문제나 학습한 내용도 정리하며 자연스럽게 공유 문화가 생겼습니다. 

작업 진행 중에도 정리한 내용을 기준 삼아 소통하는 일이 많았고, 명확한 기준과 구조를 공유하는 일이 생각보다 팀의 생산성에 큰 영향을 준다는 걸 느꼈습니다. 개발자에게 왜 소통을 중요하게 생각하는지를 실감할 수 있었던 계기가 된 것 같습니다.
...
문제를 마주할 때, 왜 이 일이 발생했는지를 이해해야 정확한 대처가 가능하다고 생각합니다. 그래서 기능 요구나 상황의 배경을 먼저 파악하여 정리한 뒤, 그중 가장 빠르게 구현할 수 있는 방법을 선택하는 편입니다. 처음부터 높은 완성도를 갖추고 싶어 하지만, 현실적인 일정 안에서는 일단 틀을 잡고, 유지보수에 유리하도록 구조를 설계해 기한 내 마무리한 뒤, 여유가 생기면 열어두었던 더 나은 방법으로 보완해나가는 쪽이 낫다는 것을 배웠습니다.

기억력이 좋은 편이 아니라 메모와 문서화를 일상처럼 하고 있고, 이것이 팀 협업에서도 큰 도움이 된다는 것을 여러 번 느꼈습니다.

말로만 공유되던 내용을 기준으로 정리해두면 반복적인 확인이나 오해를 줄일 수 있고, 다른 팀원이 작업할 때 기준점이 되어 협업도 훨씬 수월해졌습니다.

협업에서는 의견을 분명하게 표현하되, 먼저 팀원들의 생각을 충분히 듣는 걸 중요하게 생각합니다. 존중을 바탕으로 논의가 이루어질 때, 더 빠르게 합의점에 도달할 수 있다고 느꼈습니다. 중간 점검을 통해 팀원의 상황이나 진척도를 보고 배분을 조정하기도 했고, 해결이 어려운 문제는 논리적인 토론을 통해 팀의 의견을 하나로 정하기도 했습니다.

지금까지의 경험을 돌아보면, 그때그때 최선이라 생각한 방식을 습관으로 남기려 했던 것 같습니다. 앞으로도 더 나은 방법이 있다면 배우고 바꿔나가며, 더 나아지려고 할 것입니다. 저는 이런 꾸준한 개선을 향한 태도가 제 강점이라고 생각합니다.
