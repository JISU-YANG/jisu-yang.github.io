---
layout: default
title: Who I am
parent: Introduce
nav_order: 1
---


<div class="flex-justify-between mb-4" style="display: inline-flex; width:100%">
    <div>
        <img id="profile-image" width="260" />
        <script>
            ImageDeliver
                .id('profile-image')
                .category(PathResource.PROFILE)
                .file('profile_resume.JPG')
                .delivery();
        </script>
    </div>
    <div style="text-align: right; min-width: 320px">
        <h2>사색하는 개발자, 양지수 입니다.</h2>
        <h3>주니어 백엔드 개발자</h3>
        <h4 class="text-grey-dk-000" style="padding-top: 80px">Last updated: 2023.03.30</h4>
    </div>
</div>

<table>
    <tr>
        <td>
            <strong>Contact</strong>
        </td>
        <td>
            <a href="">jisuyang@kakao.com</a>
        </td>
    </tr>
    <tr>
        <td>
            <strong>Blog</strong>
        </td>
        <td>
            <a href="https://jisulog.com">https://jisulog.com</a>
        </td>
    </tr>
    <tr>
        <td>
            <strong>GitHub</strong>
        </td>
        <td>
            <a href="https://github.com/JISU-YANG ">https://github.com/JISU-YANG </a>
        </td>
    </tr>
</table>

---

# Introduce.
머리 속에 있는 것을 눈 앞에 실현하는 것만큼 가슴 설레는 일은 없는 것 같습니다.
만약 그것이 단순히 보이는 것을 넘어 어떠한 기능까지 할 수 있다면 저의 가치로부터 많은 사람들의 라이프 스타일까지 바꿀 수 있다고 생각합니다.

<br>

### 영향력있는 개발자가 되기 위한 여정에 있습니다.
- 코드의 인과관계를 논리적으로 살펴볼 수 있도록, **상황을 분리해 사고**합니다.
- 반복 작업을 줄이고 편의성을 높이는 서비스를 개발하는 **프로젝트의 경험을 많이 쌓고** 있습니다.
- 효율적이고 개선 가능한 상황을 인지할 수 있도록, **체계적으로 분류하는 습관**을 가지고 있습니다.
- 좋지 않은 기억력 대신, **문서화하고 기록된 데이터를 활용**하는 것에 익숙해지고 있습니다.
- 다양한 가능성을 사고실험을 통해, **미리 변수와 리스크를 낮출 수 있도록 고민**합니다.


--- 

# Project Experience.
## 2023
### NP Launcher
런처 프로그램
- 1
- 2
- 3

자세히보기

<br>

## 2022
### Today
스마트 대시보드 서비스
- 1
- 2
- 3

자세히보기

<br>

## 2021
### Darling
커플 서비스
- 1
- 2
- 3

자세히보기

---

### Space
프로젝트 관리 툴
- 1
- 2
- 3

자세히보기

---

### Aloha Coupon
커플 쿠폰 앱

- 1
- 2
- 3

자세히보기

<br>

## 2020
### Gola
자동 인원 선별 서비스
- 1
- 2
- 3

자세히보기

---

### Blank
자료 수집 챗봇
- 1
- 2
- 3

자세히보기

---

### Hyper
스마트 북마크 서비스
- 1
- 2
- 3

자세히보기

<br>

## 2019
### NAWA
모임 중개 서비스
- 1
- 2
- 3

자세히보기

---

### AirBooks
도서 관리 프로그램
- 1
- 2
- 3

자세히보기

---

### Coinbank
모의 가상화폐 거래소
- 1
- 2
- 3

자세히보기

<br>

## 2018
### History
약속 관리 노트
- 1
- 2
- 3

자세히보기



# Other Experience.
## Leadership.
### 크루 리더
- 아이디어를 실현하는 프로젝트 네트워크, [EGO CREW](https://ego.so) 를 2009년부터 리드하고 있습니다.
- 총 20명의 크루원들과 다양한 주제로 28번의 프로젝트 및 스터디 활동을 진행하였습니다. 
- 7개의 개발 관련 프로젝트를 진행하였고, 5개의 서비스는 웹과 앱스토어에 출시할 수 있었습니다.

### 교내 수상
  - 2019, **선행상**, 코리아 IT 아카데미  
    정규과정 시간 외 <u>야간/주말 자율학습 리드 및 교육</u>으로 타 훈련생 학습향상 도움 수여  


  - 2013, **기능상**, 종로산업정보학교  
    <u>실습시간 조교</u>로 학우들의 실습과 설명을 보조


  - 2008, **정보 통신상**, 가락초등학교  
    컴퓨터 관련 자격증 우수 취득 및 <u>정보수업 교사 보조</u>

## ETC.
### 애플 공인 스토어
**제품 판매 및 고객 경험 향상**{: .text-grey-dk-000}
- 몇 가지 호기심을 실제로 근무하며 현장에서 데이터를 통해 답을 얻었습니다.
- 앱과 액세서리를 활용하는 다양한 고객 관심사별 제품 사용의 최고의 경험을 꾸준히 고민함으로써, <u>고객 만족도가 향상</u>했고 재방문 또는 지인 소개로 이어진 적이 있습니다.
- 체계적인 응대 프로세스를 적용하여 20개의 매장 중 <u>하루 매출 최고 기록</u>(5,000만 원, 개인 비중 60%)를 달성해보았습니다.

### 프리랜서 디자이너
**편집 및 웹 디자인**{: .text-grey-dk-000}
- 머리 속 아이디어를 시각화해보는 첫 경험이었습니다.
- 브랜딩을 강조하는 디자인으로 <u>다양한 분야</u>의 클라이언트와 작업해볼 수 있었습니다.
- 300개 이상의 카테고리에 대한 분류 및 UI를 새롭게 설계하여, 상세 페이지 <u>도달 시간을 60% 이상 단축</u>한 디자인으로 공기업 웹디자인 외주를 수주했습니다.

# Skill.
<table>
    <tr>
        <td><strong>Back-End</strong></td>
        <td>
            <ul>
                <li>Java, Swift</li>
                <li>Spring Boot, Spring MVC, Electron</li>
                <li>Spring Data JPA, Querydsl, Hibernate, MyBatis</li>
                <li>Gradle, Maven</li>
                <li>IntelliJ, Visual Studio Code, Xcode</li>
            </ul>
        </td>
    </tr>
    <tr>
        <td><strong>DevOps</strong></td>
        <td>
            <ul>
                <li>MySQL, MariaDB, Oracle</li>
                <li>AWS Route53, S3, EC2</li>
                <li>Tomcat</li>
            </ul>
        </td>
    </tr>
    <tr>
        <td><strong>Front-End</strong></td>
        <td>
            <ul>
                <li>Javascript</li>
                <li>SwiftUI, HTML/CSS</li>
            </ul>
        </td>
    </tr>
</table>

---

# Education.
### 전자정부 표준프레임워크 및 공통컴포넌트 개발자
19.12 ~ 19.12 | 한국정보화진흥원

### 응용 프로그램 개발 및 빅데이터 분석 전문가양성
19.06 ~ 19.12 | Korea IT Academy

### JAVA 단과 과정
19.03 ~ 19.05 | Korea IT Academy

### 컴퓨터정보과
13.03 ~ 14.02 | 종로산업정보학교  

### 과학중점 과정
11.03 ~ 14.02 | 방산고등학교  



